...
ROWS = [
    ...
    ("kv", "Now", "CS + Math @ UMass Amherst"),
    ("kv", "Focus", "AI / ML / Systems"),
    ("kv", "Prev", "Machine Learning Intern @ JHU APL"),
    ("kv", "Edu", "CS + Math Honors '28"),

    ("gap",),

    ("sec", "Stack"),
    ("kv", "Languages", "Python, C++, Java"),
    ("kv", "AI / ML", "PyTorch, TensorFlow, LLMs, OpenAI"),
    ("kv", "Backend", "React, Next.js, Node, GraphQL"),
    ("kv", "Tools", "Docker, Linux, Git, AWS"),

    ("gap",),

    ("sec", "Highlights"),
    ("bul", "SPIE DCS 2026 Research Publication"),
    ("bul", "Autonomous Systems + GenAI @ JHU APL"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">suhas@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">suhas</tspan><tspan fill="{MUTED}">@</tspan>'
